...
void VAlignEditorDockWidget::writeBackToTcl()
{
    ...
    if (m_alignDoc) {
        // ✅ 補上 undo：這裡是 VAlignProfileView 拖曳/新增/刪除/表單套用 VIP
        //    後唯一的寫回入口，先前完全沒有進 undo stack。vipListChanged
        //    在拖曳過程中會逐畫面（mouseMoveEvent）觸發，因此比照
        //    HorizontalAlignmentEdit::movePI() 的作法，用專屬 mergeId
        //    （3）把整段拖曳合併成一筆 Undo，而非逐 pixel 各記一筆。
        const QJsonObject before = m_alignDoc->toJson();
        ...
        auto* cmd = new command::AlignmentEditCommand(
            m_alignDoc, before, m_alignDoc->toJson(), tr("編輯縱斷面 VIP"), /*mergeId=*/3);
