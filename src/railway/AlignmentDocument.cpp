                                    ...
                                    SpiralType type2)
{
    ...
    if (parentDocument())
        command::AlignmentEditCommand::push(parentDocument(), before,
                                            parentDocument()->toJson(), "Add SCS");
    ...
    for (const GroupSpec& g : groups) {
        ...
        if (cCount == 1) {
            ...
            if (totalDeflection >= M_PI - 1e-9) {
                ...
                // ── 修正：p0.radius／radius 皆為 ALD 讀檔時取絕對值後的量
                // （見 AldFileIO::readHorizontalALD），本身不帶轉向（左彎／
                // 右彎）資訊。CircularArcElement::centreXY() 卻是依「帶正負
                // 號的半徑」決定圓心落在切線哪一側（正=右彎、負=左彎，見
                // RailwayAlignmentElement.cpp）。先前直接餵入恆為正值的
                // p0.radius，等同於永遠假設是右彎：一旦實際量測到的是左彎
                // 曲線（本群組 SC→CS 方位角變化 ~180°、正是最容易忽略的邊
                // 界案例），算出的圓心會落在錯誤的一側，讓下面 addLC()/
                // addCA() 依附的 Fixed CircularArc 本身幾何就是自相矛盾的
                // （PC/PT 是量測到的正確座標，圓心卻在對面），求解器內部
                // 的二分搜尋找不到符合切線的螺線長度、直接回傳 -1，導致
                // 出螺旋（甚至入螺旋）整段被靜默略過——不會畫出來，資料表
                // 也不會顯示。
                //
                // 修正方式：兩個正負號分別試算圓心，實際比對哪一個圓心到
                // p1（量測到的圓弧終點）距離最接近半徑 R，就是真正的轉向；
                // 這個判斷不依賴 totalDeflection 是否接近 180°（那正是導致
                // 進入本分支的原因），對任意圓弧兩端量測點都成立。
                const double azStart = p0.azimuth;
                const QPointF p0xy = ptXY(p0);
                const QPointF p1xy = ptXY(p1);
                const QPointF rHat(std::cos(azStart), -std::sin(azStart));
                const QPointF centerPlus (p0xy.x() + radius * rHat.x(),  p0xy.y() + radius * rHat.y());
                const QPointF centerMinus(p0xy.x() - radius * rHat.x(),  p0xy.y() - radius * rHat.y());
                const double errPlus  = std::abs(std::hypot(p1xy.x() - centerPlus.x(),
                                                             p1xy.y() - centerPlus.y())  - radius);
                const double errMinus = std::abs(std::hypot(p1xy.x() - centerMinus.x(),
                                                             p1xy.y() - centerMinus.y()) - radius);
                const double signedRadius = (errPlus <= errMinus) ? radius : -radius;

                CircularArcElement arcElem(signedRadius);
        ...
        if (cCount == 0) {
            ...
            double radius = std::abs(sPt.radius);
            ...
            if (radius < 1e-6) {
                // 常見情況（例如線形以 CS 開頭 / SC 結尾）：緊鄰的圓弧完全
                // 在檔案資料範圍之外，半徑本來就不會被記錄。過去的做法是
                // 直接放棄整個群組——緩和曲線連同它整段一起消失，資料表／
                // 3D 顯示都少一段。現在改為在緩和曲線的建構線那一側，補上
                // 一段固定 10 公尺長的「建構弧」（Construction Arc），讓
                // 使用者至少有一個可見、可編輯的圓弧元素可事後修正，而不
                // 是整段資料憑空消失。
                //
                // 半徑：真正的原始半徑不可考，這裡借用同一份檔案裡「距離
                // 最近、且確實記錄了半徑」的圓弧關鍵點作為預設猜測值（與
                // AlignmentSolver.cpp 的 adjacentArcRadius()／"VA" 虛擬弧
                // 提示列同一套邏輯：檔案範圍外的真實半徑並未記錄，只能以
                // 鄰近實測值作為最合理的起始猜測，使用者可在資料表雙擊
                // 修改）。整份檔案都沒有任何實測半徑時，才退回舊行為
                // （記錄警告並略過）。
                double guessRadius = 0.0;
                for (int d = 1; d < n && guessRadius < 1e-6; ++d) {
                    const int fwd = sItem.ptIdx + d;
                    const int bwd = sItem.ptIdx - d;
                    if (fwd < n && std::abs(rawPts[fwd].radius) > 1e-6)
                        guessRadius = std::abs(rawPts[fwd].radius);
                    else if (bwd >= 0 && std::abs(rawPts[bwd].radius) > 1e-6)
                        guessRadius = std::abs(rawPts[bwd].radius);
                }
                if (guessRadius < 1e-6) {
                    qWarning() << "[HorizontalAlignmentEdit] seedFromRawPoints: bare"
                                  " spiral at raw point" << sItem.ptIdx << "connects to a"
                                  " virtual (off-file) circular arc (construction line"
                                  " T/C recorded), but no measured radius exists anywhere"
                                  " in this file to use as a construction-arc placeholder"
                                  " -- cannot reconstruct geometry, skipped";
                    continue;
                }

                constexpr double kConstructionArcLen = 10.0;   // 建構弧固定長度 [m]

                // 轉向：真正半徑未知，但轉向可由緩和曲線自身已記錄的方位角
                // 變化推得（正 = 右轉，符合 EditableElement.radius 號誌慣例）。
                const double az0 = sPt.azimuth;
                const double az1 = rawPts[sItem.ptIdx + 1].azimuth;
                double dAz = az1 - az0;
                while (dAz >  M_PI) dAz -= 2.0 * M_PI;
                while (dAz < -M_PI) dAz += 2.0 * M_PI;
                const double turnSign = (dAz >= 0.0) ? 1.0 : -1.0;

                // 建構弧銜接在緩和曲線「建構線那一側」的端點：
                // constructionIsBefore==true 時是緩和曲線起點 p0（線形起點，
                // 建構弧在其前）；否則是緩和曲線終點 p1（線形終點，建構弧
                // 在其後）。
                const QPointF anchorPt = constructionIsBefore ? p0 : p1;
                const double  anchorAz = constructionIsBefore ? az0 : az1;
                const QPointF center(anchorPt.x() + turnSign * guessRadius * std::cos(anchorAz),
                                      anchorPt.y() - turnSign * guessRadius * std::sin(anchorAz));
                const double delta = kConstructionArcLen / guessRadius;

                QPointF arcStart, arcEnd;
                if (constructionIsBefore) {
                    // 建構弧在緩和曲線之前：往回（逆行進方向）解算 10m 前的 PC。
                    const double azPC = anchorAz - turnSign * delta;
                    arcStart = QPointF(center.x() - turnSign * guessRadius * std::cos(azPC),
                                        center.y() + turnSign * guessRadius * std::sin(azPC));
                    arcEnd   = anchorPt;
                } else {
                    // 建構弧在緩和曲線之後：往前解算 10m 後的 PT。
                    const double azPT = anchorAz + turnSign * delta;
                    arcStart = anchorPt;
                    arcEnd   = QPointF(center.x() - turnSign * guessRadius * std::cos(azPT),
                                        center.y() + turnSign * guessRadius * std::sin(azPT));
                }
                addFixedCurve(arcStart, arcEnd, center, guessRadius);

                qWarning() << "[HorizontalAlignmentEdit] seedFromRawPoints: bare spiral"
                              " at raw point" << sItem.ptIdx << "connects to an off-file"
                              " circular arc with no recorded radius -- inserted a 10m"
                              " placeholder construction arc (R ~=" << guessRadius
                           << "m, borrowed from the nearest measured curve in this file)"
                              " instead of skipping the whole group; please verify /"
                              " correct the radius in the data table.";
                radius = guessRadius;
            }
...
// ============================================================================
//  addReverseSpiral  ─ Fixed CircularArc₁ → SpiralIn → SpiralOut → Fixed
//                       CircularArc₂  (S1><S2，中間無圓弧，曲率過零)
//
//  與 addACA() 的關鍵差異：ACA 只建立 1 個 SpiralIn 元素（Egg transition，
//  曲率單調變化、不過零，要求 R1≠R2 但同號）；本函式建立 2 個元素
//  （SpiralIn + SpiralOut），對應兩弧曲率方向相反、中間必須過零的反向緩和
//  曲線（見 ReverseSpiral_Command_實作計畫.md 第 0 節落差分析）。曲率同號
//  /相反的判定需要兩弧的行進方位角（由 solve() Pass 1 求出），因此本函式
//  只做結構性檢查（型別／Fixed／半徑非零／相鄰），與 addACA() 對 R1≈R2 的
//  即時檢查同一層級；真正的曲率方向相反判定與 1 自由度解族反解留給
//  AlignmentSolver::solveReverseSpiral()（Pass 2f，見計畫文件第 2、2.4 節）。
//
//  沿用 addACA() 的欄位覆用慣例：SpiralIn/SpiralOut 的 tangentIdxBefore /
//  tangentIdxAfter 分別存放 arc1Idx / arc2Idx（兩者皆指向 CircularArc，
//  不是 Tangent）。與 addSCS() 不同的是，這裡改呼叫既有的
//  insertElementsOrdered()（可一次處理多個新元素的索引位移，addACA() 撰寫
//  時尚未有此共用工具，故當年手動展開；本函式直接重用，屬於單純複用既有
//  基礎設施，不影響 addACA() 本身)。
// ============================================================================

int HorizontalAlignmentEdit::addReverseSpiral(const ReverseSpiralSpec& spec)
{
    const int arc1Idx = spec.arc1Idx;
    int       arc2Idx = spec.arc2Idx;

    if (arc1Idx < 0 || arc1Idx >= m_elems.size()) {
        qWarning() << "[HorizontalAlignmentEdit] addReverseSpiral: arc1Idx out of range:" << arc1Idx;
        return -1;
    }
    if (arc2Idx < 0 || arc2Idx >= m_elems.size()) {
        qWarning() << "[HorizontalAlignmentEdit] addReverseSpiral: arc2Idx out of range:" << arc2Idx;
        return -1;
    }
    if (m_elems[arc1Idx].type != EditableElementType::CircularArc) {
        qWarning() << "[HorizontalAlignmentEdit] addReverseSpiral: element at arc1Idx is not a CircularArc";
        return -1;
    }
    if (m_elems[arc2Idx].type != EditableElementType::CircularArc) {
        qWarning() << "[HorizontalAlignmentEdit] addReverseSpiral: element at arc2Idx is not a CircularArc";
        return -1;
    }
    if (m_elems[arc1Idx].mode != ConstraintMode::Fixed) {
        qWarning() << "[HorizontalAlignmentEdit] addReverseSpiral: arc1 must be Fixed (arc1Idx=" << arc1Idx << ")";
        return -1;
    }
    if (m_elems[arc2Idx].mode != ConstraintMode::Fixed) {
        qWarning() << "[HorizontalAlignmentEdit] addReverseSpiral: arc2 must be Fixed (arc2Idx=" << arc2Idx << ")";
        return -1;
    }
    if (std::abs(m_elems[arc1Idx].radius) < 1e-9 ||
        std::abs(m_elems[arc2Idx].radius) < 1e-9) {
        qWarning() << "[HorizontalAlignmentEdit] addReverseSpiral: arc radius ≈ 0";
        return -1;
    }

    const QJsonObject before = parentDocument() ? parentDocument()->toJson() : QJsonObject();

    // ── 相鄰性檢查：arc1、arc2 之間必須「緊鄰」或「僅隔一段既有 Tangent」──
    //    後者對應輸入文件描述的「STS，但 T≈0」情境：使用者原本已有一段
    //    （可能不短）切線連接兩弧，REVERSESPIRAL 的用途正是把該切線換成
    //    S1><S2。若中間夾了任何非 Tangent 元素、或超過一段元素，視為不
    //    相鄰，拒絕建立（比照 removeFloatingBetween() 對「非 Floating 缺口
    //    留著不動」的保守處理原則）。
    if (arc2Idx == arc1Idx + 1) {
        // 緊鄰，無需移除任何東西。
    } else if (arc2Idx == arc1Idx + 2 &&
               m_elems[arc1Idx + 1].type == EditableElementType::Tangent) {
        const int removePos = arc1Idx + 1;
        m_elems.remove(removePos, 1);
        // 修正所有既有元素的索引參照（比照 removeFloatingBetween() 同款
        // 位移邏輯，count=1）。
        for (auto& e : m_elems) {
            if (e.tangentIdxBefore >= removePos) --e.tangentIdxBefore;
            if (e.tangentIdxAfter  >= removePos) --e.tangentIdxAfter;
        }
        arc2Idx = removePos;   // 移除後，arc2 現在緊接在 arc1 之後
    } else {
        qWarning() << "[HorizontalAlignmentEdit] addReverseSpiral: arc1Idx" << arc1Idx
                   << "and arc2Idx" << spec.arc2Idx
                   << "are not adjacent (must be immediately consecutive, "
                      "or separated by exactly one Tangent)";
        return -1;
    }

    // ── 入螺旋 S1（接 arc1 出口，曲率 1/R1 → 0）───────────────────────────
    EditableElement spiralIn;
    spiralIn.type                  = EditableElementType::SpiralIn;
    spiralIn.mode                  = ConstraintMode::Floating;
    spiralIn.radius                = std::abs(m_elems[arc1Idx].radius);
    spiralIn.length                = 0.0;   // 未知，交由 solveReverseSpiral() 反解
    spiralIn.tangentIdxBefore      = arc1Idx;   // ← Arc₁ index（非切線，沿用 ACA 覆用慣例）
    spiralIn.tangentIdxAfter       = arc2Idx;   // ← Arc₂ index
    spiralIn.spiralType1           = spec.type1;
    spiralIn.spiralType2           = spec.type1;
    spiralIn.isReverseSpiralGroup  = true;
    spiralIn.reverseSpiralStrategy = static_cast<int>(spec.strategy);
    spiralIn.reverseSpiralParam    = spec.strategyParam;

    // ── 出螺旋 S2（接 arc2 入口，曲率 0 → 1/R2）───────────────────────────
    EditableElement spiralOut;
    spiralOut.type                 = EditableElementType::SpiralOut;
    spiralOut.mode                 = ConstraintMode::Floating;
    spiralOut.radius               = std::abs(m_elems[arc2Idx].radius);
    spiralOut.length               = 0.0;   // 未知，交由 solveReverseSpiral() 反解
    spiralOut.tangentIdxBefore     = arc1Idx;   // 與 spiralIn 相同，供 Pass 2f 配對辨識
    spiralOut.tangentIdxAfter      = arc2Idx;
    spiralOut.spiralType1          = spec.type2;
    spiralOut.spiralType2          = spec.type2;
    spiralOut.isReverseSpiralGroup = true;
    // reverseSpiralStrategy/Param 只掛在 spiralIn（比照 spiralType1/2 的
    // 「代表性元素攜帶額外資料」慣例），spiralOut 維持預設值 0/0.0。

    // 插入於 arc1Idx 之後；insertElementsOrdered() 會自動處理其餘元素與
    // 這兩個新元素本身（tangentIdxAfter=arc2Idx）的索引位移。
    const int spiralInIdx = insertElementsOrdered(arc1Idx + 1, { spiralIn, spiralOut });
