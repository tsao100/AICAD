 ...
 * @brief 文件管理器
 *
 * DocumentManager 管理應用程式中所有開啟的 CAD 文件:
 * - 建立/開啟/關閉文件
 * - 追蹤當前活動文件
 * - 管理文件生命週期
 *
...
public:
     ...
     */
    cad::Document* findDocument(const QString& fileName) const;
...
Q_SIGNALS:
     ...
     * @param document 文件指標
     */
    void documentCreated(cad::Document* document);
     ...
     * @param fileName 文件檔名
     */
    void documentClosed(const QString& fileName);
...
private Q_SLOTS:
     ...
     * @brief 處理文件檔名改變
     */
    void onDocumentFileNameChanged(const QString& fileName);
